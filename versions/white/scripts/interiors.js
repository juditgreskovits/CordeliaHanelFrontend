...
studioHanel.TeaserInteriors = function() {
	...
	function update(windowHeight, scrollTop, maxScrollTop) {
	 	...
	 	var teaserAboutHeight = $("#teaser-about").height();
	 	var aboutHeight = $("#about").height();
	 	var targetTop;
	 	var ratio;
	 	if(scrollTop < landingHeight + teaserAboutHeight) {
	 		targetTop =  $("#about").position().top + aboutHeight;
	 		ratio = 1;
	 	}
	 	else if(scrollTop > landingHeight + teaserAboutHeight + aboutHeight) {
	 		targetTop = scrollTop;
	 		ratio = 0;
	 	}
	 	else {
	 		targetTop = landingHeight + teaserAboutHeight + aboutHeight;
...
studioHanel.Interiors = function() {
	...
	function update(windowHeight, scrollTop, maxScrollTop) {
		...
		var targetTop;
		if(scrollTop < landingHeight + teaserAboutHeight + aboutHeight) {
	 		targetTop = $("#teaser-interiors").position().top + teaserInteriorsHeight;
	 	}
	 	else {
	 		targetTop = landingHeight + teaserAboutHeight + aboutHeight + teaserInteriorsHeight;
	 	}
